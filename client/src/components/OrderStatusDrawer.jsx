...
export function OrderStatusDrawer({ order, onDismiss }) {
  ...
  const displayStatus = order.status === 'Placed' ? 'Order Received' : order.status;

  return (
    <section className={`order-drawer ${expanded ? 'expanded' : ''}`} aria-label="Current order status">
      <button
      ...
      >
        ...
        <span className="drawer-hint">{expanded ? 'Collapse ↓' : 'Track order ↑'}</span>
      </button>

      <div className="order-drawer-content" aria-hidden={!expanded}>
        <OrderTracker order={order} />
        {canDismiss && <button className="secondary drawer-dismiss" onClick={onDismiss}>Close order</button>}
      </div>
    </section>
  );
}
