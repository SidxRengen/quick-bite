...
export function SnackbarProvider({ children }) {
  ...
  const showSnackbar = useCallback((message, options = {}) => {
    ...
    const timer = setTimeout(() => dismissSnackbar(id), options.duration || 4200);
    ...
    <SnackbarContext.Provider value={value}>
      ...
      <div className="snackbar-stack" aria-live="polite" aria-atomic="false">
        {snackbars.map((snackbar) => (
          <div className={`snackbar ${snackbar.variant}`} role={snackbar.variant === 'error' ? 'alert' : 'status'} key={snackbar.id}>
            <span className="snackbar-icon" aria-hidden="true">{icons[snackbar.variant] || icons.info}</span>
            <span>{snackbar.message}</span>
            <button type="button" onClick={() => dismissSnackbar(snackbar.id)} aria-label="Dismiss notification">×</button>
          </div>
        ))}
      </div>
