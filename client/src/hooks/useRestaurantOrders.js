...
export function useRestaurantOrders(enabled) {
  ...
  useEffect(() => {
    ...
    const socket = io(SOCKET_URL, {
      auth: { token: localStorage.getItem(RESTAURANT_TOKEN_KEY) },
    });
    socket.on("restaurant:order-created", (order) => {
      setOrders((current) => replaceOrder(current, order));
      showSnackbar(
        `New order #${order._id.slice(-6).toUpperCase()} from ${order.customer.name}`,
        { variant: "info", duration: 6000 },
      );
    });
    socket.on("restaurant:order-updated", (order) =>
      setOrders((current) => replaceOrder(current, order)),
    );
  ...
  const updateStatus = async (orderId, status) => {
    ...
    setError("");
    try {
      const updatedOrder = await api.updateRestaurantOrderStatus(
        orderId,
        status,
      );
