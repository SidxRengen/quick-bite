...
export function validateMenuItem(payload = {}, partial = false) {
  ...
  if (!partial || has("description")) {
    ...
    if (values.description.length < 5 || values.description.length > 300)
      errors.description = "Description must contain 5 to 300 characters";
  }
  ...
  if (!partial || has("category")) {
    ...
    if (values.category.length < 2 || values.category.length > 50)
      errors.category = "Category must contain 2 to 50 characters";
  }
  if (has("available")) {
  ...
  if (partial && !Object.keys(values).length)
    throw new HttpError(400, "Provide at least one menu field to update");
  if (Object.keys(errors).length)
    throw new HttpError(400, "Invalid menu item", errors);
  return values;
}

export const createMenuItem = (payload) =>
  MenuItem.create(validateMenuItem(payload));

export async function updateMenuItem(id, payload) {
  const item = await MenuItem.findByIdAndUpdate(
    id,
    validateMenuItem(payload, true),
